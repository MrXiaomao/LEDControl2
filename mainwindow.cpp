 ...
 * @LastEditTime: 2025-12-22 22:08:48
 * @Description: 请填写简介
 */
...
void MainWindow::on_bt_baseLineSample_clicked()
{
    ...
    //获取界面FPGA相关控制参数
    int timesLED = ui->spinBox_timesLED->value();
    ...
    // 开始基线采集
    commManager->baseLineSample_manual(timesLED);
...
{
    // 双重检查：确保串口已打开且未被禁用
    if(!commManager->getSerialPortStatus() || !ui->fanControlButton->isEnabled()) {
        QMessageBox::information(this, "提示", "请先打开串口或等待测量完成");
        return;
    }
    ...
    // 更新按钮文本
    if (m_fanStatus) {
        ui->fanControlButton->setText("关闭风扇");
        logger->info("开启风扇");
    } else {
